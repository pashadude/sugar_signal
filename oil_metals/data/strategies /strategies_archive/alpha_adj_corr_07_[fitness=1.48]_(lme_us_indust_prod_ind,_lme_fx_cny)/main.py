config = AlphaConfig()
def alpha_adj_corr_07(s: Streams) -> pd.DataFrame:
    """
    Hypothesis:
    - Adjusted correlation via RSI (lme_fx_cny and lme_us_indust_prod_ind) with ts_ffill.
    """
    corr = correlation(ts_ffill(s.lme_fx_cny), ts_ffill(s.lme_us_indust_prod_ind), window=54)
    rsi_corr = ts_rsi(corr, period=38)
