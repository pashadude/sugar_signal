config = AlphaConfig()
def alpha_adj_corr_11(s: Streams) -> pd.DataFrame:
    """
    Hypothesis:
    - Adjusted correlation via RSI (lme_cn_equity_ind and lme_eu_equity_ind) with ts_ffill.
    """
    corr = correlation(ts_ffill(s.lme_cn_equity_ind), ts_ffill(s.lme_eu_equity_ind), window=18)
    rsi_corr = ts_rsi(corr, period=19)
