config = AlphaConfig()
def alpha_open_in_stock_2(s: Streams) -> pd.DataFrame:
    """
    Hypothesis:
    - Long: lme_open_interest <= SMA (4-day) AND lme_stock_in >= SMA (2-day).
    - Short: lme_open_interest > SMA (4-day) AND lme_stock_in < SMA (2-day).
    """

    lme_open_interest=s.lme_open_interest
    lme_stock_in=s.lme_stock_in
    sma1 = ts_mean(lme_open_interest, window=4)
    sma2 = ts_mean(lme_stock_in, window=2)
