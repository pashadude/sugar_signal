config = AlphaConfig()
def alpha_open_on_stock(s: Streams) -> pd.DataFrame:
    """
    Hypothesis:
    - Long: lme_open_interest <= SMA (4-day) AND lme_on_warrant <= SMA (4-day).
    - Short: lme_open_interest < SMA (4-day) AND lme_on_warrant > SMA (4-day).
    """

    lme_open_interest=s.lme_open_interest
    lme_on_warrant=s.lme_on_warrant
    sma1 = ts_mean(lme_open_interest, window=4)
    sma2 = ts_mean(lme_on_warrant, window=4)
